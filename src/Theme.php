...
use Einice\Theme\Exceptions\ThemeException;
use Einice\Theme\Facades\Agent;
...
class Theme
{
    ...
    public function init()
    {
        ...
        $theme = $this->app['config']->get('theme.theme_name');

        if ($this->app['config']->get('theme.is_agent')) {
            if (Agent::isMobile()) {
                $theme = $this->app['config']->get('theme.mobile_name');
            }
        }
    ...
    public function asset($path, $secure = null, $version = false)
    {
        ...
        if (!$this->theme) {
            throw new ThemeException('Theme should be init first');
        }
